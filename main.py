...
from tools.models import LeNet_5
from tools.models import LeNet_5_CIFAR
from tools.huffmancoding import huffman_encode_model
...
parser.add_argument('--output', default='saved_models/weight_sharing.ptmodel', type=str,
...
accuracy = test()
torch.save(model, f"saved_models/initial_model.ptmodel")
...
print ('running time:   ', str(end - start)+'s')
torch.save(model, f"saved_models/retraining.ptmodel")
accuracy = test()
...
def apply_weight_sharing(model, bits=4):
    ...
    for module in model.children():
        ...
        min_ = min(mat.data)
        max_ = max(mat.data)
        space = np.linspace(min_, max_, num=int(2**bits))
...
model = torch.load('saved_models/retraining.ptmodel')
...
model = torch.load('saved_models/weight_sharing.ptmodel')
